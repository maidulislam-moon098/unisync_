...
-- Create the discussions table first (if it doesn't exist)
...
-- Create the discussion_comments table next
...
-- Create the discussion_upvotes table last
...
-- Create unique constraints to prevent duplicate upvotes
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT FROM pg_indexes 
        WHERE indexname = 'discussion_upvotes_user_discussion_idx'
    ) THEN
        CREATE UNIQUE INDEX discussion_upvotes_user_discussion_idx 
        ON public.discussion_upvotes (user_id, discussion_id) 
        WHERE discussion_id IS NOT NULL;
    END IF;
    
    IF NOT EXISTS (
        SELECT FROM pg_indexes 
        WHERE indexname = 'discussion_upvotes_user_comment_idx'
    ) THEN
        CREATE UNIQUE INDEX discussion_upvotes_user_comment_idx 
        ON public.discussion_upvotes (user_id, comment_id) 
...
BEGIN
    IF NOT EXISTS (
        SELECT FROM pg_indexes 
        WHERE indexname = 'discussion_upvotes_discussion_id_idx'
    ) THEN
        CREATE INDEX discussion_upvotes_discussion_id_idx 
        ON public.discussion_upvotes(discussion_id) 
        WHERE discussion_id IS NOT NULL;
    END IF;
    
    IF NOT EXISTS (
        SELECT FROM pg_indexes 
        WHERE indexname = 'discussion_upvotes_comment_id_idx'
    ) THEN
        CREATE INDEX discussion_upvotes_comment_id_idx 
        ON public.discussion_upvotes(comment_id) 
        WHERE comment_id IS NOT NULL;
    END IF;
    
    IF NOT EXISTS (
        SELECT FROM pg_indexes 
        WHERE indexname = 'discussion_upvotes_user_id_idx'
    ) THEN
        CREATE INDEX discussion_upvotes_user_id_idx 
        ON public.discussion_upvotes(user_id);
    END IF;
    
    IF NOT EXISTS (
        SELECT FROM pg_indexes 
        WHERE indexname = 'discussion_comments_discussion_id_idx'
    ) THEN
        CREATE INDEX discussion_comments_discussion_id_idx 
        ON public.discussion_comments(discussion_id);
    END IF;
    
    IF NOT EXISTS (
        SELECT FROM pg_indexes 
        WHERE indexname = 'discussions_course_id_idx'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT FROM pg_indexes 
        WHERE indexname = 'discussions_user_id_idx'
    ) THEN
        CREATE INDEX discussions_user_id_idx 
        ON public.discussions(user_id);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        AND policyname = 'discussions_select_policy'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussions' 
        AND policyname = 'discussions_insert_policy'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussions' 
        AND policyname = 'discussions_update_policy'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussions' 
        AND policyname = 'discussions_delete_policy'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussion_comments' 
        AND policyname = 'discussion_comments_select_policy'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussion_comments' 
        AND policyname = 'discussion_comments_insert_policy'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussion_comments' 
        AND policyname = 'discussion_comments_update_policy'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussion_comments' 
        AND policyname = 'discussion_comments_delete_policy'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussion_upvotes' 
        AND policyname = 'discussion_upvotes_select_policy'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussion_upvotes' 
        AND policyname = 'discussion_upvotes_insert_policy'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussion_upvotes' 
        AND policyname = 'discussion_upvotes_update_policy'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT FROM pg_policies 
        WHERE tablename = 'discussion_upvotes' 
        AND policyname = 'discussion_upvotes_delete_policy'
    ) THEN
