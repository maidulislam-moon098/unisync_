...
export default function CreateFaculty() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDebug("Starting faculty creation process...")
      ...
      setDebug(`Creating faculty with email: ${formData.email}`)
      ...
      if (authData.user) {
        ...
        setMessage({ text: "Faculty member created successfully", type: "success" })
      ...
      setMessage({ text: error.message || "Failed to create faculty member", type: "error" })
    } finally {
      setLoading(false)
    }
  }
  ...
  if (user?.role !== "admin") {
    return (
      <div className="text-center py-10">
        <h1 className="text-2xl font-semibold text-white">Create Faculty</h1>
  ...
  return (
    <div>
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-white">Create New Faculty</h1>
        <Link href="/dashboard/admin/faculty" className="text-purple-500 hover:text-purple-400">
          Back to Faculty
        </Link>
      </div>
      <p className="mt-1 text-gray-400">Add a new faculty member to the system</p>
