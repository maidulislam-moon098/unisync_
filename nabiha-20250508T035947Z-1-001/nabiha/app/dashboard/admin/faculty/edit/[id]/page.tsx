...
export default function EditFaculty() {
  ...
  useEffect(() => {
    const fetchFaculty = async () => {
      if (!user || user.role !== "admin" || !facultyId) return
      ...
      try {
        ...
        if (data) {
          ...
          setMessage({ text: "Faculty member not found", type: "error" })
        }
      } catch (error: any) {
        console.error("Error fetching faculty:", error)
        setMessage({ text: error.message || "Failed to fetch faculty data", type: "error" })
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ text: "Faculty member updated successfully", type: "success" })
      ...
      setMessage({ text: error.message || "Failed to update faculty member", type: "error" })
    } finally {
      setSubmitting(false)
    }
  }
  ...
  if (user?.role !== "admin") {
    return (
      <div className="text-center py-10">
        <h1 className="text-2xl font-semibold text-white">Edit Faculty</h1>
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-gray-400">Update faculty information</p>
